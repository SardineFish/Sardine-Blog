...
use crate::{
    email_notify::CommentNotifyInfo, post::PostServiceExtend, user::Author,
    utils::json_datetime_format,
};
...
impl<'m> CommentService<'m> {
    pub async fn get_comments_of_pid(
        ...
        pid: PidType,
        depth_limit: usize,
    ) -> Result<Vec<NestedCommentRef>> {
        let comments = self
            .inner()
            .service
            .model
            .comment
            .get_by_comment_root(pid)
            .await?;
    ...
    pub async fn post(&self, comment_to: PidType, text: &str, author: Author) -> Result<PidType> {
        let post = self.service().model.post.get_raw_by_pid(comment_to).await?;
        let user = match author {
            Author::Anonymous(info) => self.service().user().get_anonymous(&info).await?,
            Author::Authorized(auth) => self
                .service()
                .model
                .user
                .get_by_uid(&auth.uid)
    ...
    fn construct_comments(
        comments: Vec<Comment>,
        root_pid: PidType,
        depth_limit: usize,
    ) -> Result<Vec<NestedCommentRef>> {
        let mut nested_comments: HashMap<PidType, NestedCommentRef> =
            HashMap::with_capacity(comments.len());
        let mut comment_container: HashMap<PidType, PidType> =
            HashMap::with_capacity(comments.len());
        ...
        for comment in comments {
            ...
            if comment_ref.comment_to() == root_pid || depth_limit == 1 {
                ...
                if parent.depth() < depth_limit {
                    ...
                    let parent_container_pid = *comment_container
                        .get(&parent.pid())
                        .ok_or_else(|| Error::post_not_found(parent.pid()))?;

                    let grandparent = nested_comments
                        .get(&parent_container_pid)
