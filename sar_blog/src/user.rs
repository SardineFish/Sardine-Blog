...
use crate::{
    error::*,
    service::Service,
    validate::{validate_uid, Validate},
};
use chrono::Utc;
use model::{
    Access, AnonymousUserInfo, AuthenticationInfo, HashMethod, HistoryData, User, UserInfo,
};
use model::{Model, RedisCache, SessionAuthInfo, SessionID};
...
impl<'m> UserService<'m> {
    ...
    pub async fn login(
        ...
        let server_pwd_hash =
            hash_func(format!("{}{}", user.auth_info.password_hash, challenge).as_str());
    ...
    pub async fn get_auth_challenge(
        ...
        match result {
            Ok(user) => {
                self.create_challenge(session_id, &user.auth_info.salt, user.auth_info.method)
                    .await
            }
    ...
    pub async fn sign_out(&self, target_session: &SessionID, session_id: &SessionID) -> Result<()> {
        if target_session != session_id {
            ...
            let target_uid = self
                ...
                .map_service_err()?
                .ok_or_else(|| {
                    Error::DataNotFound(model::Error::SessionNotFound(target_session.to_string()))
                })?;
        ...
        if let (Some(token), Some(uid)) = (token, uid) {
            self.redis
                .access()
                .delete_session_token(&uid, session_id, &token)
                .await
                .map_service_err()?;
        }
    ...
    async fn create_fake_challenge(
        &self,
        session_id: &SessionID,
        uid: &str,
    ) -> Result<AuthChallenge> {
        if let Some(salt) = self
            ...
            .map_service_err()?
        {
            self.create_challenge(session_id, &salt, HashMethod::SHA256)
                .await
        } else {
    ...
    async fn create_challenge(
        ...
        self.redis
            ...
            .set_challenge(
                &challenge,
                self.service.option.challenge_expire.num_seconds() as usize,
            )
            .await
            .map_service_err()?;
