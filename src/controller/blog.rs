...
) -> Response<Vec<BlogPreview>> {
    service
        .blog()
        .get_preview_list(params.from, params.count)
        .await
...
async fn get_by_pid(
    ...
    let blog = service
        .blog()
        .get_by_pid(session.id(), pid.into_inner())
        .await
        .map_contoller_result()?;
...
async fn post(
    service: extractor::Service,
    auth: extractor::Auth,
    data: web::Json<BlogContent>,
) -> Response<PidType> {
    let pid = service
        .blog()
        .post(&auth.uid, data.to_owned())
        .await
        .map_contoller_result()?;
    Ok(pid)
}
...
async fn update(
    ...
    service
        .blog()
        .update(*pid, &auth.uid, data.to_owned())
        .await
        .map_contoller_result()?;
    Ok(pid.into_inner())
}
