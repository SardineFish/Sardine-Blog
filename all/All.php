...
class All
{
    ...
    public static function GetLatest($from,$count,$mysql=null)
    {
        ...
        $sql = 'SELECT `pid`as`pid`,\'note\'as`type`,`title`as`title`,`text`as`text`,`time`as`time` FROM `note` WHERE `pid` IN (SELECT `pid` FROM `posts` WHERE `type` = \'note\') AND `ignore` = 0 UNION'
