...
session_start();
    ...
    $sql="update `article` set `ignore` = 1 where `id` = ".$id." and `ignore` = 0";
    ...
    $sql = "insert into `article` (`index`, `id`, `title`, `tags`, `text`, `author`, `time`, `operate`, `ignore`) VALUES (NULL, '".$id."', '', '', '', '".$uid."', '".$time."', 'deleted', '1');";
    $result=$mysql->runSQL($sql);
    if(!$result->succeed)
    {
    ...
    if(!All_Delete($id,'article',$uid,$time))
    {
