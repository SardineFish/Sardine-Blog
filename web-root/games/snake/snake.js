...
function getRank(clear, page, count)
{
    ...
    if (!SardineFish || !SardineFish.Games)
        console.warn("组件缺失.");
    SardineFish.API.Rank.getRankedScores({ key: "snakeWeb", skip: page * 10, count })
        .then(scores =>
        {
            for (var i = 0; i < scores.length; i++)
            {
                ...
                rankItem.append(no);
                var rankName = $('<p id="rankName' + i.toString() + '" class="rankName">' + item.name + '</p>');
...
function postScore(name)
{
    if (!SardineFish || !SardineFish.Games)
        console.warn("组件缺失.");
    SardineFish.API.Rank.postScore({ key: "snakeWeb" }, { name, score: snake.length })
        .then(rank =>
        {
            $("#selfRank").text("你排在No." + (rank + 1).toString());
            getRank(true, 1, 10);
