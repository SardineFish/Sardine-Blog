...
try{
    ...
    function setCenter(jquery, ver, hor)
    {
        if (!(jquery instanceof jQuery))
        {
            throw new Error("the object must be an jQuery object.");
        }
        for (var i = 0; i < jquery.length; i++)
        {
            (function ()
            {
                var obj = jquery.get(i);
                if (!obj.complete)
                {
    ...
    UI.setCenter = setCenter;
    
    sar.Web.UI=UI;
    return sar;
}catch(ex){console.warn("SardineFish.Web.UI:"+ex.message);}
})(window.SardineFish);