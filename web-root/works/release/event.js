...
{
    imageList[id] = null;
    imagesTemplate.dataSource = imageList;
}
function buttonUploadImagesClick(e)
{
    if (imgUploading)
        return;
    imgUploading = true;
    ...
    var x = $("#imagesPicker").get(0).contentWindow.upload({
        ...
        {
            progressBar.stop();
            progressBar.animate({ width: (100 * progress) + "%" }, 200);
        },
        onComplete: function (data)
        {
            ...
            imageList[imageList.length] = { id: imageList.length, url: data.url };
            imagesTemplate.dataSource = imageList;
            SardineFish.Web.UI.setCenter($(".image"), true, false);
            imgUploading = false;
        }
    });
}
function buttonAddDetailClick(e)
{
    $("#detailTable").append($('<tr><td><div contenteditable="true"></div></td><td><div contenteditable="true"></div></td></tr>'));
}
function addEventHandle()
{
    ...
    $("#buttonUploadImages").click(buttonUploadImagesClick);
    $("#buttonAddDetail").click(buttonAddDetailClick);
}
addEventHandle();
...
HTMLTemplate.Init();
var imagesTemplate = $("#imagesTemplate").get(0);
imagesTemplate.dataSource = [{ id: 0, url: "/img/upload/default.jpg" }];